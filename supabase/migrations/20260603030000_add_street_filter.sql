-- Migration: Add p_streets filter to search_properties_advanced
-- Allows filtering properties by street name(s)
...
CREATE OR REPLACE FUNCTION public.search_properties_advanced(
  ...
  p_streets text[] DEFAULT NULL::text[]
)
RETURNS TABLE(
...
DECLARE
  ...
  WITH filtered AS (
    ...
    WHERE p.organization_id = p_organization_id
      ...
      AND (
        (p_streets IS NOT NULL AND array_length(p_streets, 1) > 0 AND p.address_street = ANY(p_streets))
        OR p_streets IS NULL
      )
