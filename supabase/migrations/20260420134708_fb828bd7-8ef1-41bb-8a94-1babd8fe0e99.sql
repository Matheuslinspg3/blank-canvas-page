...
BEGIN
  ...
  -- Verifica se telefone já está em uso por OUTRO profile (índice único global)
  IF v_clean_phone IS NOT NULL THEN
    SELECT EXISTS (
      SELECT 1 FROM public.profiles
      WHERE phone = v_clean_phone AND user_id <> v_user_id
    ) INTO v_phone_in_use;
  END IF;

  -- Atualiza organização (telefone sempre vai pra org, sem constraint única)
  ...
  -- Atualiza profile: telefone só se não estiver em uso por outro usuário
