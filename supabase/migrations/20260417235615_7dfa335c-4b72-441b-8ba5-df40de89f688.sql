...
DECLARE
  ...
  v_is_oauth boolean;
BEGIN
  -- Detectar origem: provider vem em raw_app_meta_data (gerenciado pelo Supabase)
  v_provider := COALESCE(NEW.raw_app_meta_data->>'provider', 'email');
  v_is_oauth := v_provider IS DISTINCT FROM 'email';

  -- Nome: tenta múltiplos campos comuns de OAuth (Google envia "name" e "full_name")
  ...
  IF v_is_oauth THEN
    -- ─── Fluxo OAuth (Google etc.) ───
    -- Cria estrutura mínima; usuário será obrigado a completar o onboarding.
    v_company_name := v_full_name; -- placeholder, será sobrescrito no onboarding
    ...
    -- Não cria subscription: o plano será escolhido no onboarding.
    RETURN NEW;
  END IF;

  -- ─── Fluxo email/senha (comportamento original preservado) ───
