-- Follow-up for the restored public Starter plan.
-- Keeps existing slugs/subscriptions/payments and fixes DB-side defaults/allowlists
-- so signup/onboarding/trial creation no longer coerce Starter to Essencial.
...
BEGIN
  ...
  IF v_selected_plan NOT IN ('starter', 'essencial', 'profissional', 'business') THEN
    v_selected_plan := 'starter';
  END IF;
...
BEGIN
  ...
  IF v_existing_sub_id IS NULL THEN
    ...
    IF v_plan_slug NOT IN ('starter', 'essencial', 'profissional', 'business') THEN
      v_plan_slug := 'starter';
    END IF;
...
DECLARE
  ...
  v_trial_end timestamptz;
BEGIN
  SELECT id, trial_days INTO starter_plan
  FROM public.subscription_plans
  WHERE slug = 'starter'
    AND is_active = true
    AND COALESCE(features->>'is_internal', 'false') <> 'true'
  ...
  IF starter_plan.id IS NULL THEN
