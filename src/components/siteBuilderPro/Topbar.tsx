import { ArrowLeft, Monitor, Smartphone, Undo2, Redo2, Save, Globe } from 'lucide-react';
  ...
  return (
    <div className="h-14 border-b bg-background flex items-center justify-between px-4 shrink-0">
      ...
      <div className="flex items-center gap-3">
        <Link to="/site" className="text-muted-foreground hover:text-foreground">
      ...
      >
        <ToggleGroupItem value="desktop" size="sm" className="px-2 h-7 text-xs gap-1">
          <Monitor className="w-3.5 h-3.5" /> Desktop
        </ToggleGroupItem>
        <ToggleGroupItem value="mobile" size="sm" className="px-2 h-7 text-xs gap-1">
      ...
      <div className="flex items-center gap-2">
        ...
        {state.lastSavedAt && (
          <span className="text-xs text-muted-foreground hidden sm:inline">
            Salvo às {format(state.lastSavedAt, 'HH:mm')}
          </span>
        )}
