import { DollarSign, ShoppingCart, Home, TrendingDown, Target } from "lucide-react";
...
export function SalesMetricsSection({ dateRange, filters }: Props) {
  ...
  const { data: leads } = useLeadsMetrics(dateRange, filters);

  return (
    <section className="space-y-4" id="section-sales">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <MetricCard title="Valor Total Vendas" value={formatCurrency(sales?.totalSalesValue ?? 0)} icon={<DollarSign className="h-4 w-4" />} isLoading={salesLoading} />
        ...
        <Card id="section-funnel">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm flex items-center gap-2">
              <Target className="h-4 w-4 text-accent" /> Funil Dinâmico
            </CardTitle>
          </CardHeader>
          <CardContent>
            {funnelLoading ? (
                ...
                <BarChart data={funnel || []} layout="vertical">
                  ...
                  <Tooltip 
                    ...
                    formatter={(value: number, name: string, props: any) => {
                        const total = funnel?.reduce((acc, curr) => acc + curr.count, 0) || 1;
                        const pct = ((value / total) * 100).toFixed(1);
                        return [`${value} leads (${pct}%)`, "Quantidade"];
                    }}
                  />
