...
export default function DevElements() {
  ...
  return (
    <div className="min-h-screen bg-background p-6">
      <h1 className="text-2xl font-bold mb-6">Elementos v2 — Catálogo ({entries.length})</h1>
        ...
        <div key={category} className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {elements.map((def) => {
              ...
              const Icon = def.icon;
              return (
                <div key={def.type} className="border rounded-lg overflow-hidden">
                  <div className="bg-muted/50 p-1.5 flex items-center gap-2 border-b">
                    <Icon className="w-4 h-4 text-muted-foreground" />
                    <span className="text-xs font-medium">{def.label}</span>
                    <code className="text-[10px] text-muted-foreground ml-auto">{def.type}</code>
                  </div>
