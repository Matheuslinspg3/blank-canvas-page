...
const createId = () => (typeof crypto !== "undefined" && "randomUUID" in crypto) ? crypto.randomUUID() : `${Date.now()}_${Math.random()}`;
...
export function captureAttributionFromUrl(): AttributionContext | null {
  ...
  const next: AttributionContext = { ...base, current_url: url.href, last_seen_at: nowIso(), consent_state: (localStorage.getItem(CONSENT_KEY) as any) ?? null };
...
export function getAttributionContext(): AttributionContext | null { return parseStored(); }
export function clearAttributionContext() { if (canUseDom()) localStorage.removeItem(ATTRIBUTION_STORAGE_KEY); }
...
export function getOrCreateEventId(eventName: string) {
  const key = `${eventName}:${Math.floor(Date.now() / 30000)}`;
  if (!eventIds.has(key)) eventIds.set(key, createId());
